...
# Core dataset (~300 signs from Gardiner’s Sign List)
core_hieroglyph_data = [
    ...
    {"unicode": "𓎍", "gardiner": "U28", "translit": "ᶜꜣ", "english": "fire drill"},
    ...
    {"unicode": "𓏛", "gardiner": "Y1", "translit": "mᶜꜣt", "english": "book"},
    ...
    # Aa: Unclassified
    {"unicode": "𓍑", "gardiner": "Aa1", "translit": "wᶜꜣ", "english": "prosperity"},
...
def generate_hieroglyph_image(glyph, output_path, size=(50, 50), border_size=5):
    # Create a blank white image with border
    total_size = (size[0] + 2 * border_size, size[1] + 2 * border_size)
    img = np.ones((total_size[0], total_size[1]), dtype=np.uint8) * 255  # White background
    
    # Add black border
    img[:border_size, :] = 0  # Top
    img[-border_size:, :] = 0  # Bottom
    img[:, :border_size] = 0  # Left
    img[:, -border_size:] = 0  # Right
    
    # Draw the hieroglyph (centered in the inner white area)
    font = cv2.FONT_HERSHEY_SIMPLEX
    text_size = cv2.getTextSize(glyph, font, 1, 2)[0]
    text_x = border_size + (size[1] - text_size[0]) // 2
    text_y = border_size + (size[0] + text_size[1]) // 2
    
    # Draw the glyph in black on the inner white area
    cv2.putText(img, glyph, (text_x, text_y), font, 1, (0), 2, cv2.LINE_AA)
    
    # Apply pixelation effect (downscale and upscale)
    small = cv2.resize(img, (10, 10), interpolation=cv2.INTER_NEAREST)
    pixelated = cv2.resize(small, (total_size[1], total_size[0]), interpolation=cv2.INTER_NEAREST)
    
    # Save the pixelated image
    cv2.imwrite(output_path, pixelated)
    print(f"Generated pixelated image for '{glyph}' at {output_path}")
...
def main():
    ...
    for entry in core_hieroglyph_data:
        glyph = entry["unicode"]
        # Use hex codepoint as filename (e.g., "13000.png" for 𓀀)
        filename = f"{ord(glyph):04x}.png"
        output_path = os.path.join(template_dir, filename)
        
        if not os.path.exists(output_path):
            generate_hieroglyph_image(glyph, output_path)
        else:
            print(f"Skipped '{glyph}' - file {output_path} already exists")
